...
prepare_ligand2 <- function(mol_files, out_dir = "./",
  ...
  original_wd <- getwd()
  # Ensure we return to the original directory when the function exits
  ...
  process_one <- function(mol_file) {
    ...
    if (is.null(out_dir)) {
      ...
      # Handle relative or absolute output directory paths
      if (!file.path.is.absolute(out_dir)) {
